...
The `select_if` function allows you to pass logical statements inside. For instance you can select all the string columns by passing `select_if(is.character)`. Similarly, you can add `is.numeric`, `is.integer`, `is.logical`.
...
In fact, `select_if` allows you to select based on any logical expression, not just which datatype. It is possible to select all columns which average is above 500 for instance. To avoid errors you do have to also select numeric columns only, which you can do either upfront for easier syntax, or in the same line.  
